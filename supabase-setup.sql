...
-- Workspace data table supporting multiple item types
-- Supports flexible content types via JSONB for recipe workspace functionality
...
-- Recipe Workspace Item Types:
-- item_type: 'ingredient' -> content: {name: string, quantity: number, unit: string, available: boolean}
-- item_type: 'user_preference' -> content: {preference_type: string, value: string} (e.g., "cuisine": "curry")
-- item_type: 'recipe_suggestion' -> content: {recipes: [{title, ingredients_available, ingredients_missing, instructions}]}
-- item_type: 'note' -> content: {text: string} (legacy support)
